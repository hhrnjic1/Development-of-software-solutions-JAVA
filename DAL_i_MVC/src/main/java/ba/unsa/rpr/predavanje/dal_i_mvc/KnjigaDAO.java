...
public class KnjigaDAO {
    ...
    ArrayList<Knjiga> pretraga(String pretraga){
        ...
        try {
            ...
            while (result.next()) {
                ...
                rezultat.add(new Knjiga(id,naslov,autor,isbn));
            }
        }catch (SQLException e){
            e.getMessage();
        }
    ...
    public Knjiga dodaj(Knjiga k) throws SQLException {
    ...
    public Knjiga izmjeni(Knjiga k) {
        try {
    ...
    public ObservableList<Knjiga> sveKnjige(){
        ObservableList<Knjiga> knjige = FXCollections.observableArrayList();
        try {
            ResultSet result = sveKnjigeUpit.executeQuery();
            while (result.next()) {
                knjige.add(new Knjiga(result.getInt(1),result.getString(2),result.getString(3),result.getString(4)));
            }
        }catch (SQLException e){
            e.getMessage();
        }
